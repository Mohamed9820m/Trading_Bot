...
def main():
    ...
    mt5_config = config.get('mt5_connection', {})
    trading_params = config.get('trading_parameters', {})
    bot_settings = config.get('bot_settings', {}) # For new bot parameters
    
    # Initialize the bot with config parameters
    bot = ForexScalpingBot(
        symbols=trading_params.get('symbols', ["EURUSD", "GBPUSD", "USDJPY"]),
        timeframe=getattr(mt5, f"TIMEFRAME_{trading_params.get('timeframe', 'M1')}"),
        lot_size=trading_params.get('lot_size', 0.01),
        max_positions=trading_params.get('max_positions', 1), # Changed default to 1 for safety
        profit_pips=trading_params.get('profit_pips', 15.0),
        stop_loss_pips=trading_params.get('stop_loss_pips', 10.0),
        mt5_directory=mt5_config.get('path'),
        strategies_directory=config.get('directories', {}).get('strategies_directory', 'strategies'),
        models_directory=config.get('directories', {}).get('models_directory', 'models'),
        backtest_data_period=config.get('backtest_settings', {}).get('data_period', 30),
        live_trade=trading_params.get('live_trade', False),
        login_id=mt5_config.get('login'),
        password=mt5_config.get('password'),
        server=mt5_config.get('server'),
        # Pass new bot settings from config or use defaults
        use_atr_stops=bot_settings.get('use_atr_stops', True),
        atr_sl_multiplier=bot_settings.get('atr_sl_multiplier', 1.5),
        atr_tp_multiplier=bot_settings.get('atr_tp_multiplier', 2.0),
        position_sizing_fraction=bot_settings.get('position_sizing_fraction', 0.00), # Default to 0 (fixed lot) for safety
        simulated_spread_pips=bot_settings.get('simulated_spread_pips', 0.5),
        max_spread_pips_allowed=bot_settings.get('max_spread_pips_allowed', 2.0),
        signal_aggregation_method=bot_settings.get('signal_aggregation_method', "best_strategy_or_all"),
        min_votes_for_signal=bot_settings.get('min_votes_for_signal', 1)

    )
    
    # Connect to MetaTrader
    if not bot.connect_to_metatrader():
        print("Failed to connect to MetaTrader.")
        return
    
    
    try:
        # Create and save a default strategy
        default_strategy_data = bot.create_default_strategy()
        bot.save_strategy("Default_Scalping_Strategy", default_strategy_data) # Ensure name matches
        
        # Create and save a custom strategy
        example_strategy_json = create_example_strategy() # Returns JSON string
        bot.add_custom_strategy(example_strategy_json) # add_custom_strategy handles parsing and saving
        
        # Import advanced strategies from the provided JSON
        try:
            if os.path.exists('advanced_strategies.json'):
                with open('advanced_strategies.json', 'r') as f:
                    advanced_strategies = json.load(f)
                logger.info("Importing advanced strategies...") 
                count = bot.import_advanced_strategies(advanced_strategies) # This now saves them
                logger.info(f"Imported {count} advanced strategies")
            else:
                logger.warning("advanced_strategies.json not found. Skipping import.")
        except Exception as e:
            logger.error(f"Error loading/importing advanced strategies: {e}\n{traceback.format_exc()}")
        
        # Load all strategies (including default, example, and imported ones if saved by above steps)
        bot.load_strategies() 
        
       
        
        # Run backtest on each strategy for comparison (e.g., on EURUSD)
        # This helps determine the self.best_strategy_name
        logger.info("\nRunning backtests on all strategies for EURUSD to determine best strategy...")
        backtest_results_for_best = {}
        # Ensure strategies are loaded before backtesting
        if not bot.strategies:
            logger.error("No strategies loaded. Cannot run backtests.")
        else:
            for name, strategy_config in bot.strategies.items():
                logger.info(f"Backtesting {name} for EURUSD...")
                result = bot.backtest_strategy("EURUSD", strategy_config) # Pass strategy_config
                if result:
                    backtest_results_for_best[name] = result
                    logger.info(f"  EURUSD Backtest - {name}: Trades: {result['trades']}, Win rate: {result['win_rate']:.2%}, Profit: {result['profit_loss']:.2f}")
            
            # Find best strategy based on profit from these EURUSD backtests
            best_profit_from_backtest = float('-inf')
            temp_best_strategy_name = None
            for name, result in backtest_results_for_best.items():
                if result['profit_loss'] > best_profit_from_backtest:
                    best_profit_from_backtest = result['profit_loss']
                    temp_best_strategy_name = name
            
            if temp_best_strategy_name:
                bot.best_strategy_name = temp_best_strategy_name # Set the best strategy for the bot instance
                logger.info(f"\nBest strategy for EURUSD from backtest: {bot.best_strategy_name} with profit {best_profit_from_backtest:.2f}")
            else:
                logger.warning("\nCould not determine a best strategy from EURUSD backtests.")

        # Fetch data and add indicators for demonstration (optional)
        # df_demo = bot.fetch_data("EURUSD", bot.timeframe)
        # if df_demo is not None:
        #     df_demo = bot.add_technical_indicators(df_demo) 
        #     logger.info(f"Demo Data (EURUSD) with indicators shape: {df_demo.shape}")
        
        # Run a single trading iteration for each symbol (for quick test before continuous)
        logger.info("\nRunning single trading iteration for all symbols (quick test)...")
        for symbol in bot.symbols:
            iteration_result = bot.run_single_iteration(symbol) 
            if iteration_result and iteration_result.get('actions'):
                logger.info(f"{symbol} single iteration actions ({iteration_result.get('active_strategy','N/A')}): {iteration_result['actions']}")
        
        # Option to run continuous trading
        if bot.live_trade:
            logger.info("\nStarting continuous trading (Ctrl+C to stop)...")
            bot.run_continuous(interval_seconds=10) # Adjust interval as needed
        else:
            logger.info("\nLive trading is disabled in config. Continuous trading will not start.")
            logger.info("Bot setup complete. Run with live_trade=true in config to trade.")

    except KeyboardInterrupt:
        logger.info("Bot operation interrupted by user.")
    except Exception as e:
        logger.error(f"Error in bot main operation: {e}\n{traceback.format_exc()}")
    finally:
        # Close all positions and disconnect
        logger.info("Shutting down bot...")
        bot.close_all_positions()
        bot.disconnect_from_metatrader()
